...
module.exports = {
    ...
    MessageHandler : function(lowercaseContent, msg) {
        if (lowercaseContent === prefix + 'netplay') {
            ...
            if (!netplayRole) {
                Logger.error(`Server ${msg.guild.name} does not have a valid Netplay role`);
                return;
            }
            ...
            if (!tournamentRole) {
                Logger.error(`Server ${msg.guild.name} does not have a valid NetplayTournament role`);
                return;
            }
            ...
            var tournamentRole = msg.guild.roles.find("name", "NetplayTournament");
            if (!organiserRole) {
                Logger.error(`Server ${msg.guild.name} does not have a valid Netplay TO role`);
                return;
            }
            if (!tournamentRole) {
                Logger.error(`Server ${msg.guild.name} does not have a valid NetplayTournament role`);
